...
2. **Diff-index update — ON HOLD (correction round 2):** `pyrmts-engine diffindex update -D <dataset> -k <tile-key> -R <scans-root> -o <index-root> <config>` → appends every not-yet-indexed scan in order (one adjacency changeset + composed nodes). Layout as landed at `e03c490`: `<index-root>/diffidx/<dataset>/index.json` (ordered scan labels) + `L{level}/{i}.parquet` (standard changeset rows: key cols + `{c}__a` / `{c}__b`), append-only / immutable nodes → plain R2 puts. pyrmts is replacing the sliding-window nodes with **aligned blocks + a level cap (L0-only default)**; **do not add this stage to `daily-ingest.yml` until they ping that the layout is final.** Stage 1 (MS seal) is unaffected.
...
2. **Pyramid configs + ingester** for the r2 datasets (per bucket, two configs each — §3.3; path-index writes hot + full shards — §3.2); wire `multiscan seal` into `daily-ingest.yml` (idempotent), R2 puts + mirrored D1 manifest. `diffindex update` joins only after pyrmts finalizes the layout (§4). Individuals kept (no `drop`) until the read path is proven.
