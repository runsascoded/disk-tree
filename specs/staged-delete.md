...
1. **Local data model + CLI (this stage).** `Plan`/`PlanItem`/`DeletionRun`/`DeletionBand` SQLite models + `disk-tree stage URI…` / `disk-tree staged` / `disk-tree unstage URI…` / `disk-tree dispatch [PLAN] [-f]`. The executor over `backend.delete()`, dry by default; a run + bands recorded; sizing from covering scans. Tested (dry-run + a fake backend for the real path, no network). Proves the mechanism generically, local-first. *The dispatcher is the admin — no approval gate yet.*
