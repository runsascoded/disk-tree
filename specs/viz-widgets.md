...
5. **V.5** filter plane v0 (`<FilterInput>` + level-filter helpers); segment-index artifacts + autocomplete as V.5b when a consumer needs cross-scan filtering.
6. **V.6** `@disk-tree/react/voronoi` glyphs/alt view.
