...
**Division:** cw-s3 owns the DuckDB producer + CFW reader (being upstreamed into pyrmts). disk-tree adopts pyrmts's `path-index` consolidation once the opt-in layout's config surface settles (`pyrmts:specs/multi-scan-consolidation.md` "Remaining: config surface … deferrable"). **Next step: a `specs/pyrmts-adoption.md`** scoping the disk-tree side (pyramid config for the union path-index, the constant-binCol snapshot contract, wiring `multiscan` consolidation into `daily-ingest.yml`, and the `pyrmts-cfw` read path in `site/functions`). No code yet — needs its own spec + cw coordination.
